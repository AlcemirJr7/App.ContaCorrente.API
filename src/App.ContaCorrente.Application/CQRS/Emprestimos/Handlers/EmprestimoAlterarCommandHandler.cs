...
namespace App.ContaCorrente.Application.CQRS.Emprestimos.Handlers
{
    public class EmprestimoAlterarCommandHandler : IRequestHandler<EmprestimoAlterarCommand, Emprestimo>
    {
        ...
        public async Task<Emprestimo> Handle(EmprestimoAlterarCommand request, CancellationToken cancellationToken)
        {
            ...
            {                
                emprestimo.Atualizar(request.Valor,emprestimo.Status,emprestimo.SaldoDevedor,request.TipoFinalidade,request.TipoEmprestimo,request.QtdParcelas,
                                     decimal.Zero,request.Juros,request.DataEfetivacao,request.DataRejeicao,emprestimo.FlagEstado,
                                     emprestimo.FlagProcesso,request.CorrentistaId);
